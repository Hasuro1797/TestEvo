...
import { deleteFeeds, updateFeed } from "../redux/actions/feedActions";
...
const Select = ({ items, setShow, show, feedId }) => {
	...
	const [input, setInput] = useState({
		...
		payment: false,
		image:
			"https://www.experimenta.es/wp-content/uploads/2020/01/abc-la-coleccion-de-lamparas-de-roberto-paoli-para-modo-luce-7.jpg",
	});
			...
			{show && (
				<Options>
					{items.map((element, index) => (
						<p key={index} onClick={handleClick(element)}>
							{element}
						</p>
					))}
				</Options>

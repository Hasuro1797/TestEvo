...
const FeedPage = () => {
    ...
    const handleSubmit = (e) =>{
        e.preventDefault();
        createFeed({...input, userId: user.user.id})
        setShowModal(!showModal);
        setInput({
            title: '',
            description: '',
    ...
    return (
        <FeedContainer>
            <FieldProfile>
                <h2>
                    {`Hola ${user.user.profile.name}`}
                </h2>
            </FieldProfile>
            <FieldFeed>
                <h1>Mis Publicaciones</h1>
                <Button type='button' text='agregar publicacion' handleClick={handleClick} />
                <Modal
                    state={showModal}
				...
				<Contain>
                    <Form onSubmit={handleSubmit}>
                        ...
                        <ButtonField>
                            <Boton 
                                type="submit"
                                disabled = {(input.title && input.description && input.price && input.status)? false : true}
                            >Crear</Boton>
                        </ButtonField>
                    </Form>
				</Contain>
